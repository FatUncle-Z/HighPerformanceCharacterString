...
//  testStl
//
//  Created by JunAFa on 14-12-5.
//  Copyright (c) 2014年 no2game. All rights reserved.
//

#include "No2StringBuilder.hpp"
...
public:
    ...
    double m_seconds;
public:
    static boost::posix_time::time_duration diff(boost::posix_time::time_duration start, boost::posix_time::time_duration end)
    {
        ...
        boost::posix_time::time_duration temp = end - start;
//        if ((end.total_nanoseconds()-start.total_nanoseconds())<0) {
...
//        }
    ...
    void Start() {
        m_startTime = boost::posix_time::microsec_clock::local_time().time_of_day();
    }
    void Stop() {
        m_endTime = boost::posix_time::microsec_clock::local_time().time_of_day();
...
int main(int argc, const char * argv[]) {
    No2StringBuilder<char> ansi;
    ansi.appendBack("Hello").appendBack(" ").appendLine("World !");
//    std::cout<<ansi.toString()<<std::endl;
    
    No2StringBuilder<wchar_t> wide;
    std::vector<std::wstring> cargoCult = getVector("/Volumes/development/boostproject/testStl/testStl/cargoCult.txt");
    if (!cargoCult.empty())
    {
        wide.addBack(cargoCult.begin(), cargoCult.end());
        std::wstring cargoCultString = wide.toString();
//        std::wcout<< cargoCultString<< "Length " << cargoCultString.size() <<  std::endl;
//        std::wcout<<wide.join(L"_\n")<<std::endl;
    ...
    wide.clear().addBack(cargoCult.begin(), cargoCult.end()).appendLine();
